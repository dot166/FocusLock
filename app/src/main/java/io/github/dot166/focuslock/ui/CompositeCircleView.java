...
public class CompositeCircleView extends FrameLayout {
    ...
    public void configure(float startAngle, Pair<Integer, String>[] values, int[] colors, int strokeWidth,
        ...
        for (int i = 0; i < values.length; i++) {
            total += values[i].getFirst();
            if (values[i].getFirst() > 0) {
                numValidValues++;
            }
        }
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            float sweepAngle = (values[i].getFirst() / total) * allocatedDegrees;
            pcv.setSweepAngle(sweepAngle);

            if (labels[i] != null) {
                int percentage = Math.round((values[i].getFirst() / total) * 100);
